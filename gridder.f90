...
real*8 ,parameter :: zSBgn = GridderZc - lzSml/2.0
real*8 ,parameter :: zSEnd = GridderZc + lzSml/2.0
...
do j=1,ny+1
    ...
    if (j==1) then
        Y(j) = 0.0
    else if(yNextLrgValue > ySBgn) then
...
do k=1,nz+1
    ...
    if (k==1) then
        Z(k) = 0.0
    else if(zNextLrgValue > zSBgn) then
...
if(myid==master)then
    open (unit=5,form='unformatted',file='mesh.x')
    write(5) nblocks
    write(5) nx, ny, nz

     write(5)  (((Xout(i,j,k),i=1,nx),j=1,ny),k=1,nz), &
               (((Yout(i,j,k),i=1,nx),j=1,ny),k=1,nz), &
               (((Zout(i,j,k),i=1,nx),j=1,ny),k=1,nz)
